...
import { MainBottomBox, SmallLabel, SelectBox, Select, BtnBox, MakePlanBtn } from './MainPc.styled'
        ...
        <MainBottomBox>
            ...
            <SelectBox>
                {withArr.map((value, index) => {
                    if(choiceIndex1.indexOf(index) == -1) {
                        return <Select onClick={() => isChoice1(index)} key={index} back={backColor} font={fontColor}>{value}</Select>
                    }else {
                        return <Select onClick={() => isChoice1(index)} key={index} back={'#277bc0'} font={'white'}>{value}</Select>
            ...
            <SelectBox>
                {whichArr.map((value, index) => {
                    if(choiceIndex2.indexOf(index) == -1) {
                        return <Select onClick={() => {isChoice2(index)}} key={index} back={backColor} font={fontColor}>{value}</Select>
                    }else {
                        return <Select onClick={() => {isChoice2(index)}} key={index} back={'#277bc0'} font={'white'}>{value}</Select>
                    }
                })}
            </SelectBox>

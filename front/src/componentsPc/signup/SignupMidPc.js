...
const SignupMid = ({ page }) => {
  ...
  const dupChk1 = async () => {
    ...
    if (user_id == undefined) {
      ...
      const duplicateIdResult = await axios.post("http://localhost:3000/user/duplicateId", {
        user_id,
      });
  ...
  const dupChk2 = async () => {
    ...
    if (nickname === undefined) {
      ...
      const duplicateNickNameResult = await axios.post(
        "http://localhost:3000/user/duplicateNickName",
        {
          nickname,
        }
      );
  ...
  const signUp = async () => {
    const signUpResult = await axios.post("http://localhost:3000/user/signUp", {
      user_id,
      user_pw,
      nickname,
      email,
  ...
  const login = async () => {
    ...
    const loginClick = await axios.post("http://localhost:3000/user/login", {
      user_id,
      user_pw
    },{
      headers: {
